...
class Sphere* SceneParser::parseSphere() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token); assert (!strcmp(token, "{"));
    getToken(token); assert (!strcmp(token, "center"));
    Vec3f center = readVec3f();
    ...
    return new class Sphere(center,radius,current_material);
}


class Plane* SceneParser::parsePlane() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token); assert (!strcmp(token, "{"));
    ...
    getToken(token); assert (!strcmp(token, "}"));
    assert (current_material != NULL);
    return new class Plane(normal,offset,current_material);
}


class Triangle* SceneParser::parseTriangle() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token); assert (!strcmp(token, "{"));
    getToken(token);
    ...
    getToken(token); assert (!strcmp(token, "}"));
    assert (current_material != NULL);
    return new class Triangle(v0,v1,v2,current_material);
}
...
Group* SceneParser::parseTriangleMesh() {
    ...
    while (1) {
        ...
        if (c == EOF) { break;
            ...
            assert (current_material != NULL);
            answer->addObject(new_fcount,new class Triangle(verts[f0-1],verts[f1-1],verts[f2-1],current_material));
            new_fcount++;
        } // otherwise, must be whitespace
    }
    delete [] verts;
